...
# Default loopback options: point fetch_one/fetch_avatars at a plain-HTTP
...
result = fetch_avatars(%w[aa bb], **loopback(port, max_total: 30))
...
result = fetch_avatars(['../etc', 'a b', '', 'x' * 16], **loopback(port))
...
result = fetch_avatars(%w[alice bob bad], **loopback(port))
...
t0 = Time.now
fetch_avatars(%w[a b c d], **loopback(port, total_deadline: 1, read_timeout: 5,
