...
# 1. DHHFetch.read_capped_body raises the instant the cap is crossed (and returns
# the body under it).
class FakeResponse
...
begin
  DHHFetch.read_capped_body(FakeResponse.new(['x' * 10, 'y' * 10]), max_bytes: 5)
  raise 'FAIL: read_capped_body did not raise on over-cap response'
rescue DHHFetch::BodyTooLarge
  # expected
end
assert(DHHFetch.read_capped_body(FakeResponse.new(%w[abc def]), max_bytes: 10) == 'abcdef',
       'read_capped_body returns the full body under the cap')
...
t0 = Time.now
result = fetch_profile_json('127.0.0.1', port, use_ssl: false, open_timeout: 1,
...
t0 = Time.now
result = fetch_profile_json('127.0.0.1', port, use_ssl: false, open_timeout: 0.5,
